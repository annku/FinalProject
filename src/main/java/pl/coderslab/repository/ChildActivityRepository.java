...
public interface ChildActivityRepository extends JpaRepository<ChildActivity,Long>{
	...
	@Query("select a from ChildActivity a where a.child.id = :id1 and a.activity.id= :id2")
	ChildActivity findByChildIdWhereActivityId(@Param("id1") long id1,	@Param("id2") long id2);
